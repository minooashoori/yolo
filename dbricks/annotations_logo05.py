...
INPUT_PATH_TRAIN = os.path.join(LOGO05_PATH, "annotations/xywh/train/metadata/")
INPUT_PATH_TEST = os.path.join(LOGO05_PATH, "annotations/xywh/test/metadata/")

INPUT_PATH_SCORES_TRAIN = os.path.join(LOGO05_PATH, "logo_predictions/xywh/train/")
INPUT_PATH_SCORES_TEST = os.path.join(LOGO05_PATH, "logo_predictions/xywh/test/")

OUTPUT_PATH_TRAIN = os.path.join(LOGO05_PATH, "annotations/gts_preds/xywh/train/")
OUTPUT_PATH_TEST = os.path.join(LOGO05_PATH, "annotations/gts_preds/xywh/test/")

OUTPUT_PATH_TRAIN_PARQUET = os.path.join(LOGO05_PATH, "annotations/gts_preds/parquet/xywh/train/")
OUTPUT_PATH_TEST_PARQUET = os.path.join(LOGO05_PATH, "annotations/gts_preds/parquet/xywh/test/")
...
def format_pred_boxes(boxes_list, width, height, abs):
...
if __name__ == "__main__":
    ...
    # read gt
    train_df = spark.read.parquet(INPUT_PATH_TRAIN).withColumnRenamed("boxes", "xywh_boxes").drop("uri")
    test_df = spark.read.parquet(INPUT_PATH_TEST).withColumnRenamed("boxes", "xywh_boxes").drop("uri")
